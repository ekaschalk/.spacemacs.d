...
The end of ~packages.el~ configures ~undo-spacemacs~, a personal package for
undoing spacemacs bindings for trimming down which-key options.
...
Undoing the bindings is disabled by default. Have ~undo-bindings?~ at the top of
~init.el~ set true to enable this feature.
